...
author: "Alejandra Lopez | Kelly Cardona | Giulia Sansone"
date: "2024-02-07"
output: 
  html_document:
    theme: cosmo
...
Neu12h_bed_files <- bed_files[7:9] 
...
```{r}
# peak union!
union_reference_bed <- c(Neu3h_reference_bed,Neu6h_reference_bed,Neu12h_reference_bed)
...
# pheno information
pheno <- data.frame(
  cell_type = factor(c(rep('Neu3h', 3), rep('Neu6h', 3), rep('Neu12h', 3)))
)
...
#head(res)
res_Neu6h_vs_Neu3h <- results(dds, contrast = c("cell_type", "Neu6h", "Neu3h"))
res_Neu12h_vs_Neu3h <- results(dds, contrast = c("cell_type", "Neu12h", "Neu3h"))
res_Neu12h_vs_Neu6h <- results(dds, contrast = c("cell_type", "Neu12h", "Neu6h"))
...
sign_idx_6vs3 <- which(abs(res_Neu6h_vs_Neu3h$log2FoldChange) > lfc_threshold &
                    res_Neu6h_vs_Neu3h$padj <= sign_threshold)
length(sign_idx_6vs3)

sign_idx_12vs3 <- which(abs(res_Neu12h_vs_Neu3h$log2FoldChange) > lfc_threshold &
                    res_Neu12h_vs_Neu3h$padj <= sign_threshold)
length(sign_idx_12vs3)

sign_idx_12vs6 <- which(abs(res_Neu12h_vs_Neu6h$log2FoldChange) > lfc_threshold &
                    res_Neu12h_vs_Neu6h$padj <= sign_threshold)
length(sign_idx_12vs6)
#res[sign_idx, ]
