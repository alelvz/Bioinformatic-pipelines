...
limma_trend <- topTable(fit, coef=ncol(design), number=nrow(fit$coefficients))

significant_DEGs <- limma_trend$adj.P.Val < 0.05 & abs(limma_trend$logFC) > 2
all_sig_DEGs <- sum(significant_DEGs)
all_sig_DEGs

up_degs <- limma_trend$logFC > 2 & limma_trend$adj.P.Val < 0.05
down_degs <- limma_trend$logFC < -2 & limma_trend$adj.P.Val < 0.05
sig_up_degs <- sum(up_degs)
sig_down_degs <- sum(down_degs)
sig_up_degs
sig_down_degs
