...
path <- paste(urld, "acc=GSE94892", "file=GSE94892_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
GSE94892_count <- as.matrix(data.table::fread(path, header=T, colClasses="integer"), rownames=1)
...
gds <- getGEO("GSE94892")
Meta_GSE94892 <- pData(gds$GSE94892_series_matrix.txt.gz@phenoData)
Meta_GSE94892 <- Meta_GSE94892[,c("title","characteristics_ch1","characteristics_ch1.1","condition:ch1","disease:ch1")]

Factors_GSE94892 <- Meta_GSE94892[,c("disease:ch1")]
GSE94892_count

## WE NEED BIOLOGICAL INFORMATION: GC, GENE LENGTH, CHROMOSOME,...

# Write the names
write.table(rownames(GSE94892_count),"gene_names.entrez.txt",
...
sum(rownames(GSE94892_count) %in% annotgene$Entrezgene)
head(GSE94892_count)
...
sum(rownames(GSE94892_count) %in% annotgene$Entrezgene)
sum(annotgene_filt$Entrezgene %in% rownames(GSE94892_count))
...
sum(as.character(rownames(annotgene_filt)) %in% rownames(GSE94892_count))

##  Work with the annotated genes!
GSE94892_count_filt <- GSE94892_count[rownames(GSE94892_count) %in% rownames(annotgene_filt),]
GSE94892_count_exc <-GSE94892_count[!(rownames(GSE94892_count) %in% rownames(annotgene_filt)),]
annotgene_ord <- annotgene_filt[rownames(GSE94892_count_filt ),]

sum(rownames(annotgene_ord)==rownames(GSE94892_count_filt))
...
Factors_GSE94892 <- data.frame(Meta_GSE94892 [ colnames(GSE94892_count_filt),c("disease:ch1")])
colnames(Factors_GSE94892)[1]<- "Group"
...
data_NOISEQ <- readData(data = GSE94892_count_filt,
                        length=lengthuse,
                        gc=gc,
                        biotype= biotype ,
                        chromosome = annotgene_ord[,c("Chromosome","start","end")],
                        factors = Factors_GSE94892)
...
save(data_NOISEQ,GSE94892_count_filt,annotgene_ord,file="GSE94892_step1.Rda")
...
load("GSE94892_step1.Rda") # data_NOISEQ,GSE94892_count_filt,annotgene_ord,file="GSE94892_step1.Rda")

############
# STEP 3.1.1: SET THE CLASS
############

GSE94892_DESeq2 <- DESeqDataSetFromMatrix(countData = GSE94892_count_filt,
...
smallestGroupSize <- 6
keep <- rowSums(counts(GSE94892_DESeq2) >= 10) >= smallestGroupSize
GSE94892_DESeq2_F <- GSE94892_DESeq2[keep,]
...
GSE94892_DESeq2_F<- DESeq(GSE94892_DESeq2_F)
GSE94892_res <- results(GSE94892_DESeq2_F)
GSE94892_res
resultsNames(GSE94892_DESeq2_F)
...
library(limma)
plotMA(GSE94892_res, ylim=c(-2,2))

#Interpretation?


lfcShrink(GSE94892_DESeq2_F,coef=c("Group_Dengue.hemorrhagic.fever_vs_Dengue.fever"))
res_lfcShrink <- lfcShrink(GSE94892_DESeq2_F,coef=c("Group_Other.Febrile.Illness_vs_Dengue.fever"))
...
GSE94892_DESeq2_F<- DESeq(GSE94892_DESeq2_F)
#res <- results(GSE94892_DESeq2_F, contrast=c('factorName','numeratorLevel','denominatorLevel'))
res <- results(GSE94892_DESeq2_F, contrast=c("Group","Other Febrile Illness","Dengue fever"))
res
resultsNames(GSE94892_DESeq2_F)
...
GSE94892_DESeq2_F <- DESeq(GSE94892_DESeq2_F, test="LRT", reduced=~1)
res <- results(GSE94892_DESeq2_F)

# Technical replicates?

# How to interpret the results?

plotCounts(GSE94892_DESeq2_F, gene="100287102", intgroup="Group")

## STEP 3.1.4: QC??

# How do visualize?
vsd <- vst(GSE94892_DESeq2_F, blind=FALSE)
rld <- rlog(GSE94892_DESeq2_F, blind=FALSE)
head(assay(vsd), 3)

# heatmap
library("pheatmap")
select <- order(rowMeans(counts(GSE94892_DESeq2_F,normalized=TRUE)),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(GSE94892_DESeq2_F)[,c("Group")])
colnames(df) <- "Group"
...
dge <- DGEList(counts=GSE94892_count)
